...
io.on('connection', socket => {
    ...
    socket.on('join-room', (roomId, userId, username, callback) => {
        ...
        socket.broadcast.to(roomId).emit('user-connected', userId, username, userList[roomId]);

        socket.on('disconnect', () => {
            userList[roomId] = myLib.removeByAttr(userList[roomId], 'id', userId);
            socket.broadcast.to(roomId).emit('user-disconnected', userId, username, userList[roomId]);
        });
    });
});
...
httpServer.listen(port, () => console.log(`Server running on port ${port}`)); // Do not use app.listen() here, it will not work because it will create a new web server
